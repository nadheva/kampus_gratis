<x-app-layout>
  ...
  <div class="col-xl-9">
    ...
    <div class="card border rounded-3">
        ...
        <!-- Card body START -->
        <div class="card-body">
          <form role="form text-left" action="{{url('data-berita', $berita->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
            ...
            <input type="hidden" class="form-control" name="status" value="{{ $berita->status }}" required>
            </div>
            <div class="mb-3">
            <label for="exampleFormControlSelect1">Gambar</label>
            <input type="file" class="form-control" name="gambar" value="{{ $berita->gambar }}" required>
            </div>
            <div class="mb-3">
